...
@pytest.mark.asyncio
async def test_process_environment_reference_requires_manifest_authority(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    name = "SANDBOX_TEST_PROCESS_ENV_VALUE"
    ...
    with pytest.raises(ValueError, match="must be resolved through a trusted Manifest"):
        await value.resolve()
    with pytest.raises(ValueError, match=f"binding {name!r} -> 'TOKEN' is not granted"):
