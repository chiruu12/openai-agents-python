...
class ProcessEnvValue(EnvValue):
    ...
    environment access must be granted on a trusted ``Manifest`` instance.
    """
    ...
    async def resolve(self) -> str:
        raise _process_environment_error(
            "ProcessEnvValue must be resolved through a trusted Manifest with "
...
class Manifest(BaseModel):
    ...
    def _reject_mount_credential_exposure_policy_input(cls, value: object) -> object:
        if isinstance(value, Mapping):
            ...
            if _PROCESS_ENVIRONMENT_ACCESS_KEYS.intersection(value):
                raise TypeError(
                    "Process environment access must be configured on a trusted Manifest "
                    "instance, not in manifest input."
                )
        return value
    ...
    async def resolve_environment(self) -> dict[str, str]:
        """Resolve the sandbox environment using this manifest's runtime-only authority."""
