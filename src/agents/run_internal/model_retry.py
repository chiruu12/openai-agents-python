...
async def _run_stream_attempt_in_one_task(
    ...
    """Own every stream pull in one task so async-generator context stays stable."""
    try:
        while True:
            await requests.get()
            try:
...
async def _await_model_attempt(
    ...
    *,
    ...
    await _cancel_and_drain_model_attempt_task(task, timeout_error)
            ...
            if timeout is not None:
                ...
                with (
                    ...
                    stream_owner = asyncio.create_task(
                        _run_stream_attempt_in_one_task(stream, stream_requests, stream_results)
                    )
            try:
                while True:
                    try:
                        if timeout is None:
                            ...
                            try:
                                ...
                                await _cancel_and_drain_model_attempt_task(stream_owner, error)
                                raise
                            except asyncio.CancelledError:
